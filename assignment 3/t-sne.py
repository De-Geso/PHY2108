...
tsne_data = TSNE(random_state = 42, n_components=2, perplexity=40,
	n_iter=300).fit_transform(X_tsne)

tsne_data_test = TSNE(random_state = 42, n_components=2, perplexity=40,
	n_iter=300).fit_transform(X_test)
...
X_clusters = kmeans.predict(tsne_data_test)
...
plt.scatter(tsne_data[:,0], tsne_data[:,1], s=1, c=Y_tsne, cmap="Spectral")
plt.gca().set_aspect('equal', 'datalim')
plt.colorbar(boundaries=np.arange(11)-0.5).set_ticks(np.arange(10))
plt.title('t-SNE Regression of MNIST')
plt.tight_layout()
plt.show()
...
plt.plot(tsne_data[:, 0], tsne_data[:, 1], "k.", markersize=2)
...
plt.title(
    "K-means clustering on the digits dataset (t-SNE-reduced data)"
)
